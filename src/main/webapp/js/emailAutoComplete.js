	...
	_renderHTML: function(cfg) {
		...
		if(curIndex > -1){
			...
			if(_cache.onlyFlag) {
				...
				$('.auto-tip',cfg.parentNode).css({'position':'absolute','width':"60%",'top':cfg.height+'px','left':"36%",
					'z-index':'10000','background-color':'#ffffff'});
                //$('.auto-tip',cfg.parentNode).css({'position':'relative','width':cfg.width - 2 + 'px',
                //    'border':'1px solid #ccc','z-index':'10000','background-color':'#ffffff'});
			}
